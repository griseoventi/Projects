...
select Location
	...
	, total_cases
	, total_deaths
	, (floor(total_deaths)/floor(total_cases))*100 as DeathPercentage
from PortfolioProject.dbo.covid_deth$
...
select Location
	...
	, total_cases
	, population
	, (floor(total_cases)/population)*100 as PercentPopulationInfected
from PortfolioProject.dbo.covid_deth$
where location like 'Russia' and continent is not null
order by 1,2
...
select Location
	...
	, MAX((floor(total_cases)/population))*100 as PercentPopulationInfected
from PortfolioProject.dbo.covid_deth$
-- where location like 'Russia' and continent is not null
...
select Location
	, MAX(floor(total_deaths)) as TotalDeathCount 
from PortfolioProject.dbo.covid_deth$
...
select SUM(floor(new_cases))as total_cases
	, SUM(floor(new_deaths)) as total_death
...
Select dea.continent
	, dea.location
	, dea.date
	, floor(dea.population) as population
	, vac.new_vaccinations
